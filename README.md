...
친절한 에러 메세지? **없습니다**. ~~에러가 발생하면 **여러분이 알아서** 판단하고 코드를 고치세요.~~
...
```bash
python main.py example.gsa
```

를 통해 파일을 실행하세요. 아래는 예시 결과입니다.

```bash
C:\Example\gsa-lang>python main.py example.gsa
12.0
24.0
문자열을 입력하세요 > 12
12
...
부정으로 바꿀 때는 아래와 같이 작성합니다.

```gsa
설망고방이 39호가 아니라고? 이게 사실이면.
설망고가 "안녕하세요"라고 안했다고? 이게 사실이면.
설망고가 추석망고랑 같은 짓을 안했다고? 이게 사실이면.
```
...
while문은 아래와 같이 작성합니다. while문을 작성할 때는 주의할 점은 들여쓰기를 해야한다는 점입니다. 
if문과 마찬가지로 조건문 중첩을 사용할 수 있습니다.
...
1. PR은 언제든 환영입니다.
2. 새로운 문법을 제안하시려면 issue를 남겨주세요. ~~그래도 추가는 안할겁니다~~
